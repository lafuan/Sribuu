...
|| 2026-07-08 (qa-agent run #6) | ❌ **Deploy: 37/50 failed** (60.5h+ stale), Test: ✅ | #625 — CRITICAL: Deploy still 100% blocked (update to #610) | #610 (commented with status update), #615 (commented — schema drift still unfixed) | — | **60.5h stale.** 24/24 deploy runs FAILED today. Root cause UNCHANGED: migration 0001 tries to CREATE INDEX on missing  column. _worker.ts INSERT still uses old columns , . 4x duplicate 'deploy blocked' issues now open (#588/601/610). PR #388 still open, wrong approach, merge-conflicted. Site HTTP 200, health OK. |
